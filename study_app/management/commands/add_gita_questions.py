...
class Command(BaseCommand):
    help = 'Add Bhagavad-gita Chapter 1 questions to the quiz system'
    
    def add_arguments(self, parser):
        parser.add_argument('--book-id', type=int, help='Specific book ID to add questions to')
    
    def handle(self, *args, **options):
        try:
            # Get the specific book or the first Bhagavad-gita book
            if options['book_id']:
                book = Book.objects.get(id=options['book_id'])
            else:
                book = Book.objects.filter(title__contains='Bhagavad-gita').first()
            
            if not book:
                self.stdout.write(self.style.ERROR('No Bhagavad-gita book found!'))
                return
            ...
            for i, q_data in enumerate(questions_data, 1):
                ...
                self.stdout.write(f"Added question {i}: {q_data['question_text'][:50]}...")
            
            self.stdout.write(self.style.SUCCESS(f'Successfully added {len(questions_data)} Chapter 1 questions to book ID {book.id}!'))
            
        except Book.DoesNotExist:
            self.stdout.write(self.style.ERROR('Book not found!'))
        except QuizModule.DoesNotExist:
            self.stdout.write(self.style.ERROR('Module 1 not found!'))
